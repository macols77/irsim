...
void CTestCompassController::SimulationStep(unsigned n_step_number, double f_time, double f_step_interval)
{
   ...
   * IMPORTANT!!: The code must exclusively depend of "encoder" variable */

  double incOrientation = ( encoder[1] - encoder[0] ) / fWheelsDistance;
  double incMovement = (encoder[0] + encoder[1])/2;


  m_vPosition.x += incMovement * cos(m_fOrientation + incOrientation/2);
  m_vPosition.y += incMovement * sin(m_fOrientation + incOrientation/2);
  m_fOrientation  += incOrientation;
