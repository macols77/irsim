...
void CIri1Controller::ComputeActualCell ( unsigned int un_priority )
{
    ...
    for ( int i = 0 ; i < m_seBlueLight->GetNumberOfInputs() ; i ++ )
    {
      ...
      if ( blueLight[i] > 0.0 ) {
        ...
        if (m_vecPreyNotChecked.size() > 0) {
          ...
          m_nState = 0;
          printf("Setting prey to go [%2.2f, %2.2f] with pathplanning: %d\n", m_nActualPreyGridX, m_nActualPreyGridY, m_nPathPlanningDone);
        }
  ...
  if (ground[0] == 0) {
    ...
    // update deliver status
    if (m_nDeliverStatus == 1 && !m_bIsInPrey && (m_nPathPlanningDone == 0 || (m_nPathPlanningDone == 1 && SeemsToBeInTheRightPrey()))) {
...
{
  double distance = sqrt( pow( (m_nRobotActualGridX - m_nActualPreyGridX), 2 ) + pow( (m_nRobotActualGridY - m_nActualPreyGridY), 2 ));
